...
function BlockCard({ block, index }) {
        ...
        return (
            <div style={{ marginBottom: 14, animation: `fadeIn 0.3s ease ${index * 0.08}s both` }}>
                ...
                <svg viewBox="0 0 400 66" width="100%" style={{ display: "block", aspectRatio: "400 / 66", height: "auto" }}>
                    ...
                    <text x="200" y="40" textAnchor="middle" fontFamily="'Playfair Display', serif" fontStyle="italic" fontWeight="900" fontSize="17" fill="#1a1a1a">
                        {block.content}
                    </text>
                </svg>
            </div>
                    ...
                    {branches.map((branch, idx) => {
                        ...
                        return (
                            <g key={idx} className="mm-node">
                                ...
                                {hasBreak ? (
                                    <text className="mm-label" x={p.cx} y={p.cy - 4}>
                                        <tspan x={p.cx}>{line1}</tspan>
                                        <tspan x={p.cx} dy="13">{line2}</tspan>
                                    </text>
                                ) : (
                                    <text className="mm-label" x={p.cx} y={p.cy + 4}>{branch}</text>
                                )}
                            </g>
                        );
                    })}
                    <circle cx="200" cy="150" r="46" fill="#1a1a1a" />
                    <text x="200" y="146" textAnchor="middle" fontFamily="'Playfair Display', serif" fontStyle="italic" fontWeight="900" fontSize="13" fill="#fff">
                        {centerText.length > 8 ? centerText.substring(0, 8) : centerText}
                    </text>
                    {centerText.length > 8 && (
                        <text x="200" y="160" textAnchor="middle" fontFamily="'Playfair Display', serif" fontStyle="italic" fontWeight="900" fontSize="13" fill="#fff">
                            {centerText.substring(8)}
                        </text>
                    )}
                </svg>
            </div>
                    ...
                    {edges.map((edge, idx) => {
                        ...
                        return (
                            <g key={idx}>
                                ...
                                {edge.label && (
                                    <text className="fc-note" x={labelX} y={labelY} textAnchor={textAnchor} style={{ fontFamily: "'Caveat', cursive", fontStyle: "italic", fontSize: 13, fill: "#6b7280" }}>
                                        {edge.label}
                                    </text>
                                )}
                            </g>
    ...
    if (cfg.style === "alert") {
        return (
            <div style={{
                marginBottom: 12, border: `0.5px solid ${tint(cfg.accent, 0.35)}`, borderRadius: 8,
                overflow: "hidden", animation: `fadeIn 0.3s ease ${index * 0.08}s both`,
            }}>
                <div style={{
                    background: tint(cfg.accent, 0.15), padding: "5px 10px",
                    display: "flex", alignItems: "center", gap: 6,
                    borderBottom: `0.5px solid ${tint(cfg.accent, 0.25)}`,
                }}>
                    <span style={{ color: cfg.accent, fontSize: 11 }}>{cfg.icon}</span>
                    <span style={{
                        color: cfg.accent, fontSize: 10, fontWeight: 700, letterSpacing: "0.14em",
                ...
                <div style={{
                    background: tint(cfg.accent, 0.03), padding: "10px 12px",
                    fontFamily: "'Caveat', cursive", fontSize: 16, color: "#1f2937",
                    lineHeight: 1.5, borderLeft: `3px solid ${cfg.accent}`,
                }}>
                    {block.content}
                </div>
            </div>
    ...
    // "box" — definition / example / riddle
    return (
        <div style={{
            marginBottom: 12, border: `0.5px solid ${tint(cfg.accent, 0.3)}`, borderRadius: 8,
            overflow: "hidden", animation: `fadeIn 0.3s ease ${index * 0.08}s both`,
        }}>
            <div style={{
                background: tint(cfg.accent, 0.12), padding: "5px 10px",
                display: "flex", alignItems: "center", gap: 6,
            }}>
                <span style={{ color: cfg.accent, fontSize: 11 }}>{cfg.icon}</span>
                <span style={{
            ...
            <div style={{
                background: tint(cfg.accent, 0.02), padding: "10px 12px",
                fontFamily: "'Caveat', cursive", fontSize: 16, color: "#1f2937", lineHeight: 1.5,
            }}>
                {block.content}
            </div>
        </div>
